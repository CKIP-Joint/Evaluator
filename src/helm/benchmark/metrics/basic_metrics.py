...
def normalize_text(text: str) -> str:
    ...
    return white_space_fix(remove_articles(remove_punc(lower(text)))).strip()  # remove \n left and right
...
def prefix_exact_match_multiplechoice(gold: str, pred: str) -> float:
    """
    The `prefix_exact_match_multiplechoice` metric is particularly useful in the zero-shot, multiple choice setting.
    """
    if not pred:
        return 0

    return 1 if pred.strip().startswith(gold.strip().split('.')[0]) else 0
...
def f1_score(gold: str, pred: str) -> float:
    if re.search(u'[\u4e00-\u9fff]', normalize_text(gold)): # For chinese
        ret = f_measure(set(list(normalize_text(gold))), set(list(normalize_text(pred))))
    else:
...
class BasicMetric(Metric):
    ...
    def compute_reference_metrics(
        ...
        """
        def chinese_stripper(p: str) -> str:
            if not p or len(p)==0:
                return p
            p = p.strip().split("\n")[0].strip()
            symbol_list = "[].,，。？：；、！「」（）《》　 "
            ...
            if len(p):
                while p[-1] in symbol_list:
                    p = p[:-1]
                    if not len(p):
                        break
            if len(p):
                while p[0] in symbol_list:
                    p = p[1:]
                    if not len(p):
                        break
        ...
        def compute_metrics_helper(
            ...
            if name.name == "pass":  # Calculate pass@k for HumanEval from CodeScenario.
                ...
                results = [
                    score_func((gold.output.text, gold.test_cases), chinese_stripper(pred)) for gold in code_golds for pred in preds
                ]
                ...
                score_func = cast(Callable[[Tuple[str, Optional[Dict]], str], float], score_func)  # Make mypy happy.
                code_golds = cast(List[CodeReference], golds)
                score_1 = max(score_func((gold.output.text, gold.test_cases), chinese_stripper(preds[0])) for gold in code_golds)
                score_k = max(
                    score_func((gold.output.text, gold.test_cases), chinese_stripper(pred)) for gold in code_golds for pred in preds
                )
            else:
                score_func = cast(Callable[[str, str], float], score_func)  # Make mypy happy.
                score_1 = max(score_func(gold.output.text, chinese_stripper(preds[0])) for gold in golds)
                score_k = max(score_func(gold.output.text, chinese_stripper(pred)) for gold in golds for pred in preds)
        ...
        metric_fn_mapping: Dict[str, Callable] = {
            ...
            "prefix_exact_match_multiplechoice": prefix_exact_match_multiplechoice,
