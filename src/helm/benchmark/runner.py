...
class RunSpec:
    ...
    def __change_name__(self, newname):
        object.__setattr__(self, "name", newname)
...
class Runner:
    ...
    def run_one(self, run_spec: RunSpec):
        ...
        # if model = to_be_evaluated, change run_spec.name
        import argparse
        ...
        if args.model_id:
            pattern = "model=\w+"
            original_model_name = re.search(pattern, run_spec.name).group(0)
            new_model_name = "{}={}".format(original_model_name.split('=')[0], args.model_id)
            run_spec.__change_name__(run_spec.name.replace(original_model_name, new_model_name))
        ...
        if False: #os.path.exists(os.path.join(run_path, "scenario_state.json")):
            # TODO restore scenario_state from json
            scenario_state = json.load(open(os.path.join(run_path, "scenario_state.json"), 'r'))
        else:
        ...
        write(os.path.join(run_path, "run_spec.json"), json.dumps(asdict_without_nones(run_spec), indent=2, ensure_ascii=False))

        # Write out scenario
        write(os.path.join(run_path, "scenario.json"), json.dumps(asdict_without_nones(scenario), indent=2, ensure_ascii=False))

        # Write scenario state
        write(os.path.join(run_path, "scenario_state.json"), json.dumps(asdict_without_nones(scenario_state), indent=2, ensure_ascii=False))

        write(
            os.path.join(run_path, "per_instance_stats.json"),
            json.dumps(list(map(asdict_without_nones, per_instance_stats)), indent=2, ensure_ascii=False),
        )

        write(
            os.path.join(run_path, "stats.json"), json.dumps([asdict_without_nones(stat) for stat in stats], indent=2, ensure_ascii=False)
        )
