...
def get_exact_match_multiplechoice_metric_specs() -> List[MetricSpec]:
    return get_basic_metric_specs(
        ["exact_match", "quasi_exact_match", "prefix_exact_match", "quasi_prefix_exact_match", "prefix_exact_match_multiplechoice"]
    )
...
### Mediatek Research ###
def get_drcd_spec() -> RunSpec:
    scenario_spec = ScenarioSpec(class_name="helm.benchmark.scenarios.drcd_scenario.DRCDScenario", args={})

    # define new spec func here because we use input_suffix="" instead of "\n" in AdapterSpec
    def get_generation_adapter_spec_drcd(
        instructions: str = "",
        input_noun: Optional[str] = None,
        newline_after_input_noun: bool = False,
        output_noun: Optional[str] = None,
    ...
    # use max_tokens=50 to save time. The max character length of all answers in DRCD test split is 46
    adapter_spec = get_generation_adapter_spec_drcd(input_noun=None, output_noun=None, max_tokens=50)
    ...
    return RunSpec(
        ...
        scenario_spec=scenario_spec,
        adapter_spec=adapter_spec,
        metric_specs=get_f1_metric_specs() + get_exact_match_metric_specs(),
        groups=["drcd"],
    )

def get_drcd_extractive_spec() -> RunSpec:
    scenario_spec = ScenarioSpec(class_name="helm.benchmark.scenarios.drcd_scenario.DRCDScenario", args={})

    # define new spec func here because we use input_suffix="" instead of "\n" in AdapterSpec
    def get_generation_adapter_spec_drcd_extractive(
        instructions: str = "",
        input_noun: Optional[str] = None,
        newline_after_input_noun: bool = False,
        output_noun: Optional[str] = None,
    ...
    # use max_tokens=50 to save time. The max character length of all answers in DRCD test split is 46
    adapter_spec = get_generation_adapter_spec_drcd_extractive(input_noun=None, output_noun=None, max_tokens=50)
    ...
    return RunSpec(
        ...
        adapter_spec=adapter_spec,
        metric_specs=get_f1_metric_specs() + get_exact_match_metric_specs(),
        groups=["drcd_extractive"],
    )

def get_fgc_spec(few_shot=None) -> RunSpec:
    if few_shot == 1:
        scenario_spec = ScenarioSpec(class_name="helm.benchmark.scenarios.fgc_scenario.FGC_1_Shot_Scenario", args={})
        name = "fgc_1"
    else:
        scenario_spec = ScenarioSpec(class_name="helm.benchmark.scenarios.fgc_scenario.FGCScenario", args={})
        name = "fgc"
    # define new spec func here because we use input_suffix="" instead of "\n" in AdapterSpec
    def get_generation_adapter_spec_fgc(
        instructions: str = "",
        input_noun: Optional[str] = None,
        newline_after_input_noun: bool = False,
        output_noun: Optional[str] = None,
    ...
    # use max_tokens=85 to save time. The max character length of all answers in FGC test split is 83
    adapter_spec = get_generation_adapter_spec_fgc(input_noun=None, output_noun=None, max_tokens=85)
...
def get_tcwsc_spec(few_shot=None) -> RunSpec:
    if few_shot == 5:
        scenario_spec = ScenarioSpec(class_name="helm.benchmark.scenarios.tcwsc_scenario.TCWSC_5_Shot_Scenario", args={})
        name = "tcwsc_5"
    else:
        scenario_spec = ScenarioSpec(class_name="helm.benchmark.scenarios.tcwsc_scenario.TCWSCScenario", args={})
        name = "tcwsc"
    # define new spec func here because we use input_suffix="" instead of "\n" in AdapterSpec
    def get_generation_adapter_spec_tcwsc(
        instructions: str = "",
        input_noun: Optional[str] = None,
        newline_after_input_noun: bool = False,
        output_noun: Optional[str] = None,
    ...
    adapter_spec = get_generation_adapter_spec_tcwsc(input_noun=None, output_noun=None, max_tokens=1)
...
def get_tcic_spec() -> RunSpec:
    scenario_spec = ScenarioSpec(class_name="helm.benchmark.scenarios.tcic_scenario.TCICScenario", args={})

    # define new spec func here because we use input_suffix="" instead of "\n" in AdapterSpec
    def get_generation_adapter_spec_tcic(
        instructions: str = "",
        input_noun: Optional[str] = None,
        newline_after_input_noun: bool = False,
        output_noun: Optional[str] = None,
    ...
    # yes or no in Chinese
    adapter_spec = get_generation_adapter_spec_tcic(input_noun=None, output_noun=None, max_tokens=5)
...
def get_sltp_spec() -> RunSpec:
    scenario_spec = ScenarioSpec(class_name="helm.benchmark.scenarios.sltp_scenario.SLTPScenario", args={})

    # define new spec func here because we use input_suffix="" instead of "\n" in AdapterSpec
    def get_generation_adapter_spec_sltp(
        instructions: str = "",
        input_noun: Optional[str] = None,
        newline_after_input_noun: bool = False,
        output_noun: Optional[str] = None,
    ...
    # yes or no in Chinese
    adapter_spec = get_generation_adapter_spec_sltp(input_noun=None, output_noun=None, max_tokens=30)
...
def get_ttqa_spec(few_shot=None) -> RunSpec:
    if few_shot == 5:
        scenario_spec = ScenarioSpec(class_name="helm.benchmark.scenarios.ttqa_scenario.TTQA_5_Shot_Scenario", args={})
        name = "ttqa_5"
    else:
        scenario_spec = ScenarioSpec(class_name="helm.benchmark.scenarios.ttqa_scenario.TTQAScenario", args={})
        name = "ttqa"
    # define new spec func here because we use input_suffix="" instead of "\n" in AdapterSpec
    def get_generation_adapter_spec_ttqa(
        instructions: str = "",
        input_noun: Optional[str] = None,
        newline_after_input_noun: bool = False,
        output_noun: Optional[str] = None,
    ...
    # yes or no in Chinese
    adapter_spec = get_generation_adapter_spec_ttqa(input_noun=None, output_noun=None, max_tokens=20)
...
def get_ttqa_mc_spec(method: str) -> RunSpec:
    scenario_spec = ScenarioSpec(
        class_name="helm.benchmark.scenarios.ttqa_scenario.TTQA_MC_Scenario",
        args={}
    )
    ...
    return RunSpec(
        name=f"ttqa_mc:method={method}",
        scenario_spec=scenario_spec,
        adapter_spec=adapter_spec,
        metric_specs=get_exact_match_multiplechoice_metric_specs(),
        groups=["ttqa_mc"],
    )
    
def get_tcic_mc_spec(method: str) -> RunSpec:
    scenario_spec = ScenarioSpec(
        class_name="helm.benchmark.scenarios.tcic_scenario.TCIC_MC_Scenario",
        args={}
    )

    adapter_spec = get_multiple_choice_adapter_spec(
        method=method,
        instructions="請閱讀以下短文，並在空格內填入正確的成語：",
        input_noun="內容",
        output_noun="答案",
        max_tokens=5,
    )
...
def get_lambada_spec() -> RunSpec:
    scenario_spec = ScenarioSpec(class_name="helm.benchmark.scenarios.lambada_scenario.LAMBADAScenario", args={})

    # define new spec func here because we use input_suffix="" instead of "\n" in AdapterSpec
    def get_generation_adapter_spec_lambada(
        instructions: str = "",
        input_noun: Optional[str] = None,
        newline_after_input_noun: bool = False,
        output_noun: Optional[str] = None,
    ...
    # use max_tokens=85 to save time. The max character length of all answers in FGC test split is 83
    adapter_spec = get_generation_adapter_spec_lambada(input_noun=None, output_noun=None, max_tokens=3)
...
def get_flud_spec(method: str) -> RunSpec:
    scenario_spec = ScenarioSpec(
        class_name="helm.benchmark.scenarios.flud_scenario.FLUDScenario",
        args={}
    )
...
CANONICAL_RUN_SPEC_FUNCS: Dict[str, Callable[..., RunSpec]] = {
    ...
    ## mediatek research
    "drcd": get_drcd_spec,
    "drcd_extractive": get_drcd_extractive_spec,
    "fgc": get_fgc_spec,
    "fgc_1": partial(get_fgc_spec, 1),
    "tcwsc": get_tcwsc_spec,
    "tcwsc_5": partial(get_tcwsc_spec, 5),
    "tcic": get_tcic_spec,
    "tcic_mc": get_tcic_mc_spec,
    "sltp": get_sltp_spec,
    "ttqa": get_ttqa_spec,
    "ttqa_mc": get_ttqa_mc_spec,
    "ttqa_5": partial(get_ttqa_spec, 5),
    "lambada": get_lambada_spec,
    "flud": get_flud_spec,
}
