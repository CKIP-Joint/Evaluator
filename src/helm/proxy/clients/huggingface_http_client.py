...
class HuggingFaceServer:
    def __init__(self, model_name, inference_endpoint: str):
    ...
    def serve_request(self, raw_request: Dict[str, Any]):
        ...
        response = requests.post(
                        ...
                        json={
                            "text": [raw_request["prompt"]],
                            **self.args['generate_kwargs'], 
                            **relevant_raw_request
                        },
                    )
        output = response.json()
        sequences = torch.Tensor(output['sequences']).type(encoded_input['input_ids'].dtype).to(self.device)
        scores = [torch.Tensor(k).to(self.device) for k in output['scores']]
...
class HuggingFaceClient(Client):
    INFERENCE_ENDPOINT: str = "http://127.0.0.1:5000/generate/"
    ...
    def get_model_server_instance(self, model_engine) -> HuggingFaceServer:
        if model_engine not in self.model_server_instances:
            # HF BLOOM 
            if model_engine == "bloom_1b1":
                self.model_server_instances[model_engine] = HuggingFaceServer("/proj/gpu_d_98001/huggingface/hub/models--bigscience--bloom-1B1/snapshots/1e718be072aa40714c9dc34e35ea6b64979a65ad", HuggingFaceClient.INFERENCE_ENDPOINT)
            elif model_engine == "bloom_3b":
                self.model_server_instances[model_engine] = HuggingFaceServer("/proj/gpu_d_98001/huggingface/hub/models--bigscience--bloom-3B/snapshots/515ae965cc83b9ebbf0054de106c434bd4ec35dc", HuggingFaceClient.INFERENCE_ENDPOINT)
            elif model_engine == "bloom_7b1":
                self.model_server_instances[model_engine] = HuggingFaceServer( "/proj/gpu_d_98001/huggingface/hub/models--bigscience--bloom-7B1/snapshots/850ba1758a7744fedae78caadc152625133b1677", HuggingFaceClient.INFERENCE_ENDPOINT)
            
            # HF BLOOMZ
            elif model_engine == "bloomz_1b1":
                self.model_server_instances[model_engine] = HuggingFaceServer("/proj/gpu_d_98001/huggingface/hub/models--bigscience--bloomz-1B1", HuggingFaceClient.INFERENCE_ENDPOINT)
            elif model_engine == "bloomz_3b":
                self.model_server_instances[model_engine] = HuggingFaceServer("/proj/gpu_d_98001/huggingface/hub/models--bigscience--bloomz-3B", HuggingFaceClient.INFERENCE_ENDPOINT)

            # HF Codegen
            elif model_engine == "codegen_2b_nl":
                self.model_server_instances[model_engine] = HuggingFaceServer( "/proj/gpu_d_98001/huggingface/hub/models--Salesforce--codegen-2B-nl/snapshots/5e06b1a66d42091757983a5b549ff100743f3904", HuggingFaceClient.INFERENCE_ENDPOINT)
            ...
            # TO-BE-EVALUATED
                ...
                if args.load_path:
                    self.model_server_instances[model_engine] = HuggingFaceServer(args.load_path, HuggingFaceClient.INFERENCE_ENDPOINT)
                elif os.environ.get('EVALUATED_MODEL_PATH'):
                    self.model_server_instances[model_engine] = HuggingFaceServer(os.environ.get('EVALUATED_MODEL_PATH'), HuggingFaceClient.INFERENCE_ENDPOINT)
                else:
