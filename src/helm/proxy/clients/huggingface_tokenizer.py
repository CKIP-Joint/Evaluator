...
def get_tokenizer(hf_tokenizer_name, local_files_only, use_fast):
    _id = threading.get_ident()
    tokenizer = TOKENIZER.get(id, None)
    if tokenizer is None:
        tokenizer = AutoTokenizer.from_pretrained(hf_tokenizer_name, local_files_only=local_files_only, use_fast=use_fast)
    ...
    def get_tokenizer(tokenizer_name: str) -> Any:
        ...
        def load_tokenizer(hf_tokenizer_name: str):
            ...
            try:
                ...
                return get_tokenizer(hf_tokenizer_name, local_files_only=True, use_fast=True)
            except OSError:
                ...
                return get_tokenizer(hf_tokenizer_name, local_files_only=False, use_fast=True)
        ...
        if tokenizer_name not in HuggingFaceTokenizers.tokenizers:
            with htrack_block(f"Loading {tokenizer_name} with Hugging Face Transformers"):
                ...
                elif tokenizer_name == "huggingface/bloom":
                    hf_tokenizer_name = "/proj/gpu_d_98001/huggingface/hub/models--bigscience--bloom-1B1/snapshots/1e718be072aa40714c9dc34e35ea6b64979a65ad"
                elif tokenizer_name == "huggingface/codegen":
                    hf_tokenizer_name = "/proj/gpu_d_98001/huggingface/hub/models--Salesforce--codegen-6B-nl/snapshots/dff91c0aea702edbea3528344d01d8b9aaee6e39"
                else:
